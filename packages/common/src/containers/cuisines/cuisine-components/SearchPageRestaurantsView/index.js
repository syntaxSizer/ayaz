...
import { formatPrice } from 'common/src/utils';
import { useSelector, useDispatch } from 'react-redux';
import Image from 'next/image';
import { toggleModalView } from 'common/src/data/store/reducers/cart/actions';
...
const FeatureSection = ({ products, padding, aisle, currency }) => {
        ...
        {products.map(item => {
          return (
            ...
            >
              ...
              <Text
                className="text"
                content={`KSh.${formatPrice(item.price)}`}
              />
              <Text
                className="header"
                content={item.name
