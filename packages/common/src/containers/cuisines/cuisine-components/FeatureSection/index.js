...
const FeatureSection = ({
  ...
  const getRestaurantsData = () => {
    const cookie = window.JSON.parse(getCookie('_cuisine') || 'null');
    setIsloading(true);
    let param = null;
    if (cuisine) {
      param = cuisine;
  ...
  return (
    <FeatureSectionWrapper pt={padding}>
      ...
      <div className="sectionHeader">
        ...
        <h2 className="sectionTitle">{titleText}</h2>
      </div>
      <div className="mainSection">
          ...
          <span key={items.length} style={{display: items.length === 0? 'intial': 'none'}}>
            <NotYet />
          </span>
