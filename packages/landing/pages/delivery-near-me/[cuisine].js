...
const DeliveryNearMeCuisine = ({areas, cuisine, restaurants}) => {
  ...
  return (
    <ThemeProvider theme={appTheme}>
      <>
        <Head>
          <title>{cuisine[0].toUpperCase() + cuisine.substr(1).split("-").join(" ")} Delivery Near Me • Ayazona</title>
          ...
          <link rel="canonical" href={`https://www.ayazona.com/delivery-near-me/${cuisine}`} />
          <meta name="title" content={`${cuisine.charAt(0).toUpperCase() + cuisine.slice(1).split("-").join(" ")} Delivery Near Me • Ayazona`} />
          <meta property="og:title" content={`${cuisine.charAt(0).toUpperCase() + cuisine.slice(1).split("-").join(" ")} Delivery Near Me • Ayazona`} />
          <meta property="og:image" content="/static/scripts/open_graph_logo.jpg" />
          <meta property="og:image:width" content="1200" />
          <meta property="og:image:height" content="628" />
          <meta property="og:url" content={`https://www.ayazona.com/delivery-near-me/${cuisine}`} />
          ...
          <meta name="twitter:title" content={`${cuisine.charAt(0).toUpperCase() + cuisine.slice(1).split("-").join(" ")} Delivery Near Me • Ayazona`} />
          ...
          <meta name="twitter:url" content={`https://www.ayazona.com/delivery-near-me/${cuisine}`} />
        ...
        <AppWrapper>
          ...
          >
            ...
            <Banner header={`${cuisine.charAt(0).toUpperCase() + cuisine.slice(1).split("-").join(" ")} Delivery Near Me`} subText={`Enter your delivery address, browse menus from the best affordable ${cuisine.split("-").join(" ")} restaurants in your neighborhood, and order delivery from the places that are open now, near you.`}/>
          </ContainerWithBackgroundImage>
          <MobileDownloadApp />
          {/* <Steps/> */}
          {/* <RestaurantSlider baseUrl={"/delivery"} cuisine={cuisine} rows={restaurants}/> */}
          <LocationBlock city={"nairobi"} baseUrl={"/delivery"} urlExtention={cuisine} locations={areas} header={"Food Delivery & Takeaway in Your City"} marginTop="40"/>
          <Slider baseUrl={"/delivery-near-me"} header="Top Cuisines" cuisines={cuisines} />
          <LocationDescription 
            header={`Use Ayazona To Try Nearby ${cuisine.charAt(0).toUpperCase()+cuisine.slice(1).split("-").join(" ")} Dishes`}
            description={`Now that you’ve figured out what you want to order for your today's soiree, just type in “${cuisine.charAt(0).toUpperCase()+cuisine.slice(1).split("-").join(" ")} Dishes Delivery Near Me” on the Ayazona website or the app for your mobile device, and browse through the myriad of options to find the place that has exactly what you have a taste for. Aside from menus, you can check prices, find out how long it takes for the driver to deliver to your door, and read reviews from other ${cuisine} dishes lovers just like you. Now all you need to do is figure out the headcount, order accordingly, and enjoy the day outside.`}/>
          <FloatingCards />
