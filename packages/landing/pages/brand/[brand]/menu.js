...
import Banner from "common/src/containers/brand/brand-menu-components/Banner";
import FeatureSection from "common/src/containers/brand/brand-menu-components/FeatureSection";

import { DrawerProvider } from "common/src/contexts/DrawerContext";
import { brands, API } from "common/src/data/brand/FeatureSection";
import restaurantAPI from "common/src/data/restaurants";
import { getBrandMenuPageSchema } from "common/src/utils";
...
export async function getStaticProps({params}) {
  const { brand } = params;
  const branches = await API.getBrandBranches(brand.split("-").join(" ").split("'").join(""));
  return {
    props: {
      brandData: getBrandData(brand),
      restaurant: branches.length ? await restaurantAPI.getRestaurant(branches[0].Restaurant.id): null,
    },
  }
}
...
const Brand = ({brandData, restaurant}) => {
  if (!restaurant) {
    return <></>
  }
  const menu = restaurant.RestaurantMenu;
  ...
  return (
    <ThemeProvider theme={appTheme}>
      <>
        <Head>
          <title>{`${brandData ? brandData.text: "Ayazona"} Menu, Prices • Ayazona`}</title>
          <meta
            name="description"
            content={`Menu and prices for ${brandData ? brandData.text: "Ayazona"}. Browse ${brandData ? brandData.text: "Ayazona"}'s menu items, find a location and get delivery with Ayazona.`}
          />
          <link rel="canonical" href={`https://www.ayazona.com/brand/${brandData.text}/menu`} />
          <meta name="title" content={`${brandData ? brandData.text: "Ayazona"} Menu, Prices • Ayazona`} />
          <meta property="og:title" content={`${brandData ? brandData.text: "Ayazona"} Menu, Prices • Ayazona`} />
          <meta property="og:image" content="/static/scripts/open_graph_logo.jpg" />
          <meta property="og:image:width" content="1200" />
          <meta property="og:image:height" content="628" />
          <meta property="og:url" content={`https://www.ayazona.com/brand/${brandData.text}/menu`} />
          <meta property="og:type" content="website" />
          <meta property="og:description" content={`Menu and prices for ${brandData ? brandData.text: "Ayazona"}. Browse ${brandData ? brandData.text: "Ayazona"}'s menu items, find a location and get delivery with Ayazona.`} />
          <meta name="twitter:title" content={`${brandData ? brandData.text: "Ayazona"} Menu, Prices • Ayazona`} />
          <meta name="twitter:description" content={`Menu and prices for ${brandData ? brandData.text: "Ayazona"}. Browse ${brandData ? brandData.text: "Ayazona"}'s menu items, find a location and get delivery with Ayazona.`} />
          ...
          <meta name="twitter:url" content={`https://www.ayazona.com/brand/${brandData.text}/menu`} />
          <meta name="theme-color" content="#002524"/>
          <script
			      key={`brand-menu-${brandData.text}-index`}
			      type="application/ld+json"
			      dangerouslySetInnerHTML={{ __html: JSON.stringify(getBrandMenuPageSchema(
              `https://www.ayazona.com/brand/${brandData.text}/menu`,
        ...
        <AppWrapper>
          ...
          <Banner titleContent={`${brandData.text} Menu`} brand={brandData}/>
          <FeatureSection menu={menu} currency={currency} />
