...
import Banner from "common/src/containers/brand/location-components/Banner";
import FeatureSection from "common/src/containers/brand/location-components/FeatureSection";
...
export async function getStaticProps({params}) {
  const { brand, location } = params;
  return {
    props: {
      brandData: getBrandData(brand),
      location,
      brandBranches: await API.getBrandBranches(brand.split("-").join(" ").split("'").join(""))
    },
  }
}
...
export async function getStaticPaths() {
  const paths = brands.map(b => {
    ...
    return {params: {brand: URL[URL.length -1], location: "nairobi"}}
  })
  return { paths, fallback: true }
}

const Location = ({brandData, brandBranches, location}) => {
  if (!brandData) {
    return <></>
  }
  ...
  return (
    <ThemeProvider theme={appTheme}>
      <>
        <Head>
          <title>{`${brandData ? brandData.text: "Ayazona"} Delivery Nairobi • Order Online • Ayazona`}</title>
          <meta
            name="description"
            content={`Order from ${brandData ? brandData.text: "Ayazona"} Nairobi, for home delivery or pickup. Select a location near you and fill up your cart - we'll handle the rest. `}
          />
          <link rel="canonical" href={`https://www.ayazona.com/brand/${brandData.text}/${location}`} />
          <meta name="title" content={`${brandData ? brandData.text: "Ayazona"} Delivery Nairobi • Order Online • Ayazona`} />
          <meta property="og:title" content={`${brandData ? brandData.text: "Ayazona"} Delivery Nairobi • Order Online • Ayazona`} />
          <meta property="og:image" content="/static/scripts/open_graph_logo.jpg" />
          <meta property="og:image:width" content="1200" />
          <meta property="og:image:height" content="628" />
          <meta property="og:url" content={`https://www.ayazona.com/brand/${brandData.text}/${location}`} />
          <meta property="og:type" content="website" />
          <meta property="og:description" content={`Order from ${brandData ? brandData.text: "Ayazona"} Nairobi, for home delivery or pickup. Select a location near you and fill up your cart - we'll handle the rest. `} />
          <meta name="twitter:title" content={`${brandData ? brandData.text: "Ayazona"} Delivery Nairobi • Order Online • Ayazona`} />
          <meta name="twitter:description" content={`Order from ${brandData ? brandData.text: "Ayazona"} Nairobi, for home delivery or pickup. Select a location near you and fill up your cart - we'll handle the rest. `} />
          ...
          <meta name="twitter:url" content={`https://www.ayazona.com/brand/${brandData.text}/${location}`} />
          <meta name="theme-color" content="#002524"/>
          <script
			      key={`delivery-${brandData.text}-location-index`}
			      type="application/ld+json"
			      dangerouslySetInnerHTML={{ __html: JSON.stringify(
              getBrandPageSchema(brandData.text)
            )}}
		      />
        </Head>
        ...
        <AppWrapper>
          ...
          <Banner brand={brandData} location={location}/>
          <FeatureSection branches = {brandBranches} brand={brandData} />
          <GroceryCards />
          <FloatingCards />
